    ...
    data: {
		...
		datasets: [{
			...
			foreach ($month as $date ){
				$S = "select * from nps where team_id =$id and date = '$date'";
				$ss = $cont-> query ($S);
		...
		label: 'Promoter',
			...
			$S = "select * from nps where team_id = $id and nps_rating >= 9 and date = '$date' ";
			$ss = $cont-> query ($S);
			echo $ss->num_rows . ',';  
		...
		label: 'Detractor',
			...
			$S = "select * from nps where team_id = $id and nps_rating <= 6 and date = '$date' ";
			$ss = $cont-> query ($S);
			echo $ss->num_rows . ',';  
