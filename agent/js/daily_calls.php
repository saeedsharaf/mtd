...
var myLineChart = new Chart(saeed, {
    ...
    data: {
		...
		datasets: [{
			...
			$S = "select * from nps where agent_id='$id' and date = '$date' ";
			$ss = $cont-> query ($S);
			echo $ss->num_rows . ',';  
		...
		label: 'Promoter',
			...
			foreach ($month as $date ){
			$S = "select * from nps where agent_id='$id' and nps_rating >= 9 and date = '$date' ";
			$ss = $cont-> query ($S);
			echo $ss->num_rows . ',';  
		...
		label: 'Detractor',
			...
			foreach ($month as $date ){
			$S = "select * from nps where agent_id='$id' and nps_rating <= 5 and date = '$date' ";
			$ss = $cont-> query ($S);
			echo $ss->num_rows . ',';  
