...
<?php
if($_GET['acw_not']){
	$sql="select * from mar where acw > 5 ";
}else if($_GET['acw']){
	$sql="select * from mar where acw > 5 ";
}else if($_GET['adh_not']){
	$sql="select * from mar where adherance < 95 ";
} else if($_GET['adh']){
	$sql="select * from mar where adherance >= 95 ";
}else if($_GET['aht_not']){
	$sql="select * from mar where aht > 260 ";
}else if($_GET['aht']){
	$sql="select * from mar where aht < 260 ";
}else if($_GET['attiude_not']){
	$sql="select * from mar where attiude < 1 ";
}else if($_GET['attiude']){
	$sql="select * from mar where attiude > 0 ";
}else if($_GET['compl_not']){
	$sql="select * from mar where compliance > 0 ";
}else if($_GET['compl_not']){
	$sql="select * from mar where compliance < 1 ";
}else if($_GET['ctb_not']){
	$sql="select * from mar where ctb > 0 ";
}else if($_GET['ctb']){
	$sql="select * from mar where ctb < 1 ";
}else if($_GET['ctc_not']){
	$sql="select * from mar where ctc > 0 ";

}else if($_GET['ctc']){
	$sql="select * from mar where ctc < 1 ";

}else if($_GET['hold_not']){
	$sql="select * from mar where hold > 3 ";

}else if($_GET['hold']){
	$sql="select * from mar where hold < 3 ";
}else if($_GET['nc_not']){
	$sql="select * from mar where nc > 0 ";

}else if($_GET['nc']){
	$sql="select * from mar where nc < 1 ";
}else if($_GET['nps_not']){
	$sql="select * from mar where nps < 45 ";
} else if($_GET['nps']){
	$sql="select * from mar where nps > 45 ";
}else if($_GET['outbound_not']){
	$sql="select * from mar where outbound > 300 ";
}else if($_GET['outbound']){
	$sql="select * from mar where outbound < 300 ";
}else if ($_GET['overpromise_not']){
	$sql="select * from mar where over_promising > 0 ";
} else if($_GET['overpromise']){
	$sql="select * from mar where over_promising < 1 ";
}else if($_GET['perfomance']){
	$sql="select * from mar where manger_id >= 111 and manger_id < 444 ORDER BY final_score DESC ";
}


else{
	$sql="select * from mar order by final_score desc ";
}
