...
if($id < 111){
$sql="select * from oct where sv_id = '$id'";
...
if($result->num_rows > 0){
	...
	$sql="select * from oct where login_id = '$idd'";
$result=$cont->query($sql);
...
*/
	...
	$sql="select * from oct where manger_id = '$id'";
$result=$cont->query($sql);
if($result->num_rows > 0){
	while($row = $result->fetch_assoc()){
	$acd_time += $row['acd_time'];
...
<!---------------------------------------------------------------------------------------------------------------------- -->
	...
	<div clas="aht" style="background-color: #f39817ab; height: 100px; width:210px; margin-right: 50px; position: relative; ">
			<div style="height: 100% ; background-color: #f39817; width: 45%">
				...
				<span class="content" style="top:30; left: 130 ; font-size: 20px;"> <?php echo round($adherence,1) ?> % </span>
