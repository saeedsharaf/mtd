...
$sql = "select * from nps where manger_id = '$id' ";
	
			$result = $cont->query($sql);
			$totalsurvey =  $result->num_rows ;
	
			$promoter = "select * from nps where manger_id = '$id' and nps_rating >= 9 "; // select all promoter rows that's higher tahn 9
			$passive = "select * from nps where manger_id = '$id' and nps_rating > 6 and nps_rating < 9 "; // select passive rows 
			$detractor = "select * from nps where manger_id= '$id' and nps_rating <= 6 "; // select detractor rows  
			
			$cs_ttb = "select * from nps where manger_id = '$id' and sats > 3 ";
			$cs_bb = "select * from nps where manger_id = '$id' and sats < 2";
			
			
			$fcr = "select * from nps where manger_id= '$id' and ir = '1' ";
			$ir = "select * from nps where manger_id= '$id' and ir = '2' ";
			...
			$ag_ttb = "select * from nps where manger_id = '$id' and agent_satisfaction > 3 "; // select agent tob box 
			$ag_bb = "select * from nps where manger_id= '$id' and agent_satisfaction < 2 "; // select agent bottom box 
